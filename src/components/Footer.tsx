      ...
      <div className="max-w-6xl mx-auto px-6 py-16 md:py-24">
        ...
        <div className="mb-16 rounded-2xl border border-background/10 bg-background/5 p-8 md:p-12">
          <div className="max-w-2xl">
            ...
            <Link
              to="/newsletter"
              className={`inline-flex items-center justify-center rounded-xl px-8 py-3 font-bold text-white transition-all btn-animate btn-glow ${
                location.pathname === '/ink' 
                  ? "bg-[hsl(210,60%,50%)] hover:bg-[hsl(210,60%,60%)] shadow-[0_4px_14px_0_rgba(0,30,87,0.4)]" 
                  : "bg-red-600 hover:bg-red-700 shadow-[0_4px_14px_0_rgba(192,58,30,0.4)]"
              }`}
            >
              Join the newsletter
            </Link>
        ...
        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 text-center">
          {/* Contact */}
          <div>
          ...
          <div>
            ...
            <nav className="flex flex-col gap-2" aria-label="Footer navigation links">
              {footerLinks.map((link) => (
                <Link key={link.path} to={link.path} className={`block cursor-pointer text-sm text-background/75 transition-colors hover:underline ${
                  location.pathname === '/ink' ? "hover:text-[hsl(210,60%,60%)]" : "hover:text-red-400"
                }`}>
                  {link.label}
                </Link>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 items-center">
              <a href="https://www.linkedin.com/company/the-unlearning-school" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 text-background/75 transition-colors text-sm ${
                  location.pathname === '/ink' ? "hover:text-[hsl(210,60%,60%)]" : "hover:text-red-400"
                }`}>
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                LinkedIn
              </a>
              <a href="https://www.facebook.com/theunlearningschool" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 text-background/75 transition-colors text-sm ${
                  location.pathname === '/ink' ? "hover:text-[hsl(210,60%,60%)]" : "hover:text-red-400"
                }`}>
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Facebook
              </a>
              <a href="https://www.instagram.com/the.unlearning.school" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 text-background/75 transition-colors text-sm ${
                  location.pathname === '/ink' ? "hover:text-[hsl(210,60%,60%)]" : "hover:text-red-400"
                }`}>
              ...
              <a href="https://api.whatsapp.com/message/SZZCJNB5GSGJP1?autoload=1&app_absent=0" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 text-background/75 transition-colors text-sm ${
                  location.pathname === '/ink' ? "hover:text-[hsl(210,60%,60%)]" : "hover:text-red-400"
                }`}>
        ...
        <div className="mt-24 overflow-hidden text-center flex flex-col items-center gap-8">
          ...
          <div className="space-y-6">
            <p className={`text-xl font-bold tracking-wide md:text-3xl ${
              location.pathname === '/ink' 
                ? "text-[hsl(210,60%,50%)]" 
                : "text-red-600"
            }`}>
